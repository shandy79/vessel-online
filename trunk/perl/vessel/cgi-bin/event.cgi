...
<tr><td class="lbl">Host:</td><td><input type="text" name="source" size="16" maxlength="24" class="txt" /></td></tr>
<tr><td class="lbl">Host URL:</td><td><input type="text" name="source_url" size="48" maxlength="256" class="txt" /></td></tr>
<tr><td class="lbl">Description:</td><td><textarea name="description" rows="4" cols="60"></textarea></td></tr>
<tr><td class="lbl">Date &amp; Time:</td><td><select name="day">
HTML

  foreach $key (sort keys %Vessel::wkdy) {
    print qq{  <option value="$key">$Vessel::wkdy{$key}</option>\n};
  }

  print <<HTML;
  </select>&nbsp;&nbsp;
  <input type="text" name="pubDate" size="20" maxlength="19" class="txt" /> (mm/dd/yyyy hh:mi am|pm)</td></tr>
...
sub ProcessForm {
  ...
  if (! $usr) {
    ...
    return;
  } elsif ($cgi->param('pubDate') !~ /^\d{2}\/\d{2}\/\d{4} \d{2}:\d{2} [ap]m$/i) {
    &Vessel::InputError("You entered an improperly formatted date or time.<br />\n");
    return;
  }

  # AddPost performs file protection prior to file append
  $id = &PostFile::AddPost('ev', "$eml ($usr)", $cgi);

  print <<HTML;
<b>SUCCESS!</b>  <a href="mailto:$eml">$usr</a>, your event has been added.<br />
Here is a link to your <a href="$Vessel::cgiRoot${this}?id=$id">event</a>.
...
sub PrintOneItem {
  ...
  $links{'Archive'} = "$Vessel::cgiRoot${this}";
  $links{'New Item'} = "$Vessel::cgiRoot${this}?id=new";
  $links{'Next'} = "$Vessel::cgiRoot${this}?id=" . ($id + 1) if ($id < &PostFile::GetMaxItems('ev'));
...
HTML
  ...
  &PostFile::PrintAllComments('ec', $id);
  &PostFile::PrintCommentForm('ec', $id, $usr, $eml);
...
sub PrintAllItems {
  $links{'New Item'} = "$Vessel::cgiRoot${this}?id=new";
  &Vessel::PrintMainOpen('i', %links);
  print "<h1>Vessel Events Archive</h1><hr />\n";

  &PostFile::PrintDateSortedPosts('ev', $this);
  return;
}
